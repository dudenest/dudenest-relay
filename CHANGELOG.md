...
- **B5 is now enforced during binary-only updates.** Startup provisioning installs Google Chrome on Debian/Ubuntu amd64, points `/usr/local/bin/dudenest-browser` at it, purges open-source Chromium packages, enables `dudenest-tz-sync`, and rewrites the kiosk service to use the neutral launcher. Remote-Hand now fails closed if only Chromium is available, instead of silently using a known-bad fingerprint for real Google accounts.

### Fixed
- **Fresh paired relays can add the first cloud account from standby.** Standby mode now returns an authenticated empty `/providers` list instead of `503`, so Flutter shows the Add Account empty state. Remote-Hand endpoints `/relay/oauth3/start|end|input` are also registered in standby, so relay-assisted login can add the first provider instead of being caught by the standby 503 fallback.
- **Browser LAN discovery preflight is allowed.** Relay CORS now includes `Access-Control-Allow-Private-Network: true`, so Flutter Web served from `https://dudenest.com` can probe `http://192.168.x.x:8086/pairing/info` under Chrome Private Network Access checks.
- **Fresh relay installs no longer crash-loop before hub bootstrap.** `relay serve` now tolerates a missing `/etc/dudenest/gdrive_client_secret.json` and starts in bootstrap-only standby, so `/health`, `/pairing/info`, `/relay/announce`, and `/relay/bootstrap` stay reachable while the hub delivers credentials. OAuth endpoints return `503` until credentials exist, and `install.sh` restarts `dudenest-relay` after successful provisioning so the freshly written OAuth config is loaded.
- **Legacy `/providers` is protected by the same owner + relay-token auth as `/files`.** Full `relay serve` now registers browser/auth/provider routes with `requireAuthWithReg`, so `/providers` rejects foreign JWTs and missing/invalid `X-Relay-Token` once the relay owner is known.
- **Relay WebSocket and noVNC proxy are no longer public on known relay URLs.** `/ws` and `/vnc/*` now use the same owner + relay-token checks as `/files`; browser-only clients can pass short-lived `token` + `relay_token` query params, while legacy query-JWT without relay token is limited to `/files/*` media compatibility.
- **Fleet fast-update no longer hits GitHub API from every relay.** Hub ping already includes the latest release asset URL; relays now pass that URL into `dudenest-relay-update.service` and `relay update` downloads it directly, avoiding GitHub API `403` rate limits that left `relay-demo` stuck on `v0.24.2` while the hub kept saying `update_now=true`. The updater also handles immutable `/usr/local/bin/relay` (`chattr +i`) by temporarily clearing and restoring the bit around the atomic replace.
- **Remote-Hand sidecar provisioning no longer marks broken deps as ready.** Startup provisioning now writes the `.provisioned-*` marker only after sidecar files, noVNC files, apt tools, and Python imports (`nacl`, `pytesseract`) are actually ready. Existing markers no longer short-circuit dependency repair, apt installs refresh package indexes first, and pip install prefers `python3 -m pip` with `pip3` fallback. This fixes the relay-poc2 pattern where Flutter stayed on `working...` because `rh_sidecar.py` crashed with `ModuleNotFoundError: No module named 'nacl'`.
- **B5 Chrome install now rejects half-installed packages.** Debian/Ubuntu hosts can leave `google-chrome-stable` in `install reinstreq half-installed` after interrupted dpkg I/O; provisioning now repairs dpkg/apt first, requires `install ok installed` plus executable Chrome, and reinstalls Chrome if the package is unhealthy instead of treating `dpkg -s` success as enough.
- **Google Chrome apt key setup is noninteractive.** Re-running B5 on a host with a partial keyring no longer fails with `gpg: cannot open '/dev/tty'`; provisioning uses batch/yes mode and recreates the source/keyring pair when either side is missing.
- **Stale fast-update environment cannot downgrade a relay.** The hub-provided `DUDENEST_RELAY_*` systemd environment is cleared after use, and `relay update` now ignores a target version that is not newer than the running binary, preventing an old manager environment from replacing `v0.24.6` with `v0.24.5` on restart.
...
### Changed
- **Method-3 anti-detection (B4/B5).** Google was disabling accounts signed in through the relay — the cause was the automation fingerprint, not the (residential) IP. Fixes: **(B4)** the OAuth browser now reuses a **persistent** `--user-data-dir` (`/var/lib/dudenest/remotehand/profile`) instead of a throwaway per-session dir, so cookies and Google's "trusted device" survive across logins (stale `Singleton*` locks are cleared, cookies kept). **(B5)** the fleet installs **real Google Chrome** on amd64 Debian/Ubuntu and adds a **timezone-from-IP** sync (`dudenest-tz-sync` oneshot + daily timer) so the browser's JS timezone matches the egress IP's geolocation. Chromium fallback was later removed entirely because silent fallback is unsafe for real-account OAuth.
